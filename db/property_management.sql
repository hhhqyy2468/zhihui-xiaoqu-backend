...
(
    ...
    contract_status  tinyint        default 1                 null comment '合同状态：1-进行中 2-已到期 3-已终止',
    sign_date        date                                     null comment '签订日期',
    terminate_date   date                                     null comment '终止日期',
    ...
    update_by        varchar(64)    default ''                null comment '更新人',
    update_time      datetime       default CURRENT_TIMESTAMP null on update CURRENT_TIMESTAMP comment '更新时间',
    constraint contract_no
        unique (contract_no)
)
    comment '车位租赁合同表';

create index idx_application_id
    on parking_rental_contract (application_id);

create index idx_contract_no
    on parking_rental_contract (contract_no);

create index idx_contract_status
    on parking_rental_contract (contract_status);

create index idx_deleted
    on parking_rental_contract (deleted);

create index idx_end_date
    on parking_rental_contract (end_date);

create index idx_owner_id
    on parking_rental_contract (owner_id);

create index idx_parking_space_id
    on parking_rental_contract (parking_space_id);
...
(
    ...
    update_by            varchar(50)                              null comment '更新人',
    update_time          datetime       default CURRENT_TIMESTAMP null on update CURRENT_TIMESTAMP comment '更新时间',
    service_rating       tinyint                                  null comment '服务评分 1-5星',
    response_rating      tinyint                                  null comment '响应速度评分 1-5星',
    professional_rating  tinyint                                  null comment '专业程度评分 1-5星',
    overall_rating       tinyint                                  null comment '总体评分 1-5星',
    comment              varchar(500)                             null comment '业主评价意见',
    rating_time          datetime                                 null comment '评价时间',
    constraint order_no
        unique (order_no)
)
...
create index idx_overall_rating
    on repair_order (overall_rating);

create index idx_rating_time
    on repair_order (rating_time);

create index idx_service_rating
    on repair_order (service_rating);
...
(
    ...
    create_time              datetime       default CURRENT_TIMESTAMP null comment '创建时间',
    update_time              datetime       default CURRENT_TIMESTAMP null on update CURRENT_TIMESTAMP comment '更新时间',
    password_status          tinyint        default 0                 null comment '支付密码状态：0-未设置，1-已设置',
    constraint user_id
        unique (user_id)
)
    comment '虚拟钱包表';
