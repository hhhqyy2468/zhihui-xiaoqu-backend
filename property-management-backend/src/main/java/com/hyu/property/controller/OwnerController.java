...
public class OwnerController {
    ...
    public AjaxResult export(@RequestBody OwnerVO owner) {
        log.info("导出业主数据, owner: {}", owner);
        List<OwnerVO> list = ownerService.selectOwnerList(owner);
        return AjaxResult.success("导出成功", ownerService.exportOwner(list));
    }
     ...
     */
    @PostMapping("/import")
    @PreAuthorize("@ss.hasPermi('property:owner:import')")
    public AjaxResult importData(MultipartFile file, boolean updateSupport) throws Exception {
        log.info("导入业主数据, filename: {}, updateSupport: {}", file.getOriginalFilename(), updateSupport);
        String result = ownerService.importOwner(file, updateSupport);
    ...
    public AjaxResult importTemplate() {
        log.info("获取业主导入模板");
        return AjaxResult.success("获取成功", ownerService.importTemplate());
    }
